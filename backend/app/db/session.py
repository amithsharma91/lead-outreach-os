...
Uses SQLite by default; the DATABASE_URL env var can point at any SQLAlchemy
URL (e.g. PostgreSQL) for a future migration.
"""
...
def ensure_schema(target_engine: Engine | None = None) -> None:
    """Safely initialize or migrate a database (SQLite only).

    - Fresh, missing, or empty SQLite database: the complete schema is
      created from the model definitions. Idempotent by construction
      (create_all skips objects that already exist).
    - Existing database: only executes ALTER TABLE ADD COLUMN and
      CREATE UNIQUE INDEX IF NOT EXISTS for objects missing from an
      existing schema. NEVER drops or destroys data.
    - Unsupported backend (non-SQLite): rejected with a clear error so the
      system never appears to support a database it cannot initialize.
    """
    eng = target_engine if target_engine is not None else engine
    if eng.dialect.name != "sqlite":
        raise RuntimeError(
            "Unsupported database backend '%s': this application supports "
            "SQLite only. No schema initialization is performed for "
            "non-SQLite databases." % eng.dialect.name
        )
    ...
    if not inspector.has_table("leads"):
        # Fresh or empty database: create the complete schema from the
        # model definitions (PR-B bootstrap). Idempotent: create_all
